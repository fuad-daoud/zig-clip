 ...
 * 
 * This example demonstrates a basic Wayland client that creates a visible window using xdg-shell protocol.
 * Build with: gcc -o main.out src/main.c src/xdg-shell.c -lwayland-client -lrt
 */
...
create_buffer(struct client_state *state, int width, int height) {
    ...
    for (int i = 0; i < width * height; i++) {
        *pixel++ = 0xFFAA00FF; // Purple color (fully opaque)
    }
